...
void loop()
{
	if (!iotClient.connected()) {
		...
		if (now - iotlastSendTime > 15000) {
			...
			if (counterFirstfloorUpdated) {
				if (addAmp) { iotString = iotString + String("&"); }
				iotString = iotString + String("field4=") + String(counterFirstfloorFlow / 1000);
				addAmp = true;
				counterFirstfloorUpdated = false;
			}
			if (counterSecondfloorUpdated) {
				if (addAmp) { iotString = iotString + String("&"); }
				iotString = iotString + String("field5=") + String(counterSecondfloorFlow / 1000);
				addAmp = true;
				counterSecondfloorUpdated = false;
			}
	...
	if (debouncerCollectorPin.update()) {
		if (debouncerCollectorPin.rose()) {
			...
			counterCollectorUpdated = true;
			float b = (3600000.0 / (now - counterCollectorTime));
			counterCollectorFlow = b * 1000.0;
			Serial.print("Count/Collector: ");
			Serial.println(counterCollectorCount);
			Serial.print("Flow/Collector: ");
			Serial.print(counterCollectorFlow / 1000);
			Serial.print(".");
			Serial.println(counterCollectorFlow % 1000);
	...
	if (debouncerWarmfloorPin.update()) {
		if (debouncerWarmfloorPin.rose()) {
			...
			counterWarmfloorUpdated = true;
			float b = (3600000.0 / (now - counterWarmfloorTime));
			counterWarmfloorFlow = b * 1000.0;
			Serial.print("Count/Warmfloor: ");
			Serial.println(counterWarmfloorCount);
			Serial.print("Flow/Warmfloor: ");
			Serial.print(counterWarmfloorFlow / 1000);
			Serial.print(".");
			Serial.println(counterWarmfloorFlow % 1000);
	...
	if (debouncerFirstfloorPin.update()) {
		if (debouncerFirstfloorPin.rose()) {
			...
			counterFirstfloorUpdated = true;
			float b = (3600000.0 / (now - counterFirstfloorTime));
			counterFirstfloorFlow = b * 1000.0;
			Serial.print("Count/Firstfloor: ");
			Serial.println(counterFirstfloorCount);
			Serial.print("Flow/Firstfloor: ");
			Serial.print(counterFirstfloorFlow / 1000);
			Serial.print(".");
			Serial.println(counterFirstfloorFlow % 1000);
	...
	if (debouncerSecondfloorPin.update()) {
		if (debouncerSecondfloorPin.rose()) {
			...
			counterSecondfloorUpdated = true;
			float b = (3600000.0 / (now - counterSecondfloorTime));
			counterSecondfloorFlow = b * 1000.0;
			Serial.print("Count/Secondfloor: ");
			Serial.println(counterSecondfloorCount);
			Serial.print("Flow/Secondfloor: ");
			Serial.print(counterSecondfloorFlow / 1000);
			Serial.print(".");
			Serial.println(counterSecondfloorFlow % 1000);
